"""
Risk Zone Visualization for NEW DATA Test Images with FINAL ViT-Tiny Model
Using FINAL ViT-Tiny classifier trained on expanded dataset
...
Model: FINAL_vit_tiny_classifier.pth (Val Acc: 82.85%)
"""
...
def load_vit_model(checkpoint_path, device='cuda'):
    """Load trained ViT model - FINAL Tiny model uses simple timm.create_model"""
    # FINAL Tiny model is just timm ViT without custom wrapper
    ...
    print(f"✅ Loaded FINAL Tiny model from: {checkpoint_path}")
...
if __name__ == "__main__":
    ...
    VIT_CHECKPOINT = 'detectors/FINAL_vit_tiny_classifier.pth'  # FINAL ViT-Tiny MODEL
    TEST_DIR = 'new_data_test_images'  # 5 test images from new_data
    OUTPUT_DIR = 'outputs/FINAL_newdata_tiny_risk_zones'
    
    # Run visualization with FINAL ViT-Tiny model on new_data test images
